// All-in-one filter demo: 검색/필터/정렬/저장/요약/무한스크롤/스켈레톤/상세패널
(function () {
  const app = document.querySelector('[data-app-all]');
  if (!app) return;
  ...
  const pillsEl = app.querySelector('[data-pills]');
  ...
  const detailClose = app.querySelector('[data-detail-close]');

  const STORAGE_KEY = 'filter_all_recent';
  ...
  const setLoading = (flag = true) => {
    state.loading = flag;
    const exist = app.querySelector('.skeletons');
    if (flag) {
      if (exist) exist.remove();
      const sk = createSkeletons(6);
  ...
  const renderPills = () => {
    ...
    if (q) pills.push({ type:'keyword', label:`검색: ${q}` });
    if (state.category !== 'all') pills.push({ type:'category', label:`카테고리: ${state.category}` });
    state.tags.forEach(t => pills.push({ type:'tag', value:t, label:`태그: ${t}` }));
    pillsEl.innerHTML = pills.map(p => `<span class="pill" data-pill="${p.type}" data-value="${p.value||''}">${p.label}<button type="button" aria-label="제거">×</button></span>`).join('');
  };
  ...
  pillsEl.addEventListener('click', e => {
    ...
    if (type === 'keyword') { state.keyword = ''; searchEl.value = ''; }
    if (type === 'category') { state.category = 'all'; categoryEl.value = 'all'; }
    if (type === 'tag') { state.tags.delete(val); tagsEl.querySelector(`[data-tag="${val}"]`)?.classList.remove('is-active'); }
    state.page = 1;
    render();
  });
